<template>
  <div class="holdings-value">
    <span class="holdings-amount">{{
      currentHoldings.primary | currency(currencies.primary)
    }}</span
    ><br />
    <span :class="{ evolution: true, decrease: lastMovement < 0 }"
      >{{ lastChange | abs | currency(currencies.primary) }} ({{
...
export default {
  name: "holdings-value",
  computed: {
    ...mapGetters(["globalHoldingsHistory", "holdingsHistory", "currencies"]),
    currentHoldings() {
    ...
    lastMovement() {
      ...
      return (
        (holdingsSum[holdingsSum.length - 1] /
          holdingsSum[holdingsSum.length - 3]) *
          100 -
        100
      )
    },
