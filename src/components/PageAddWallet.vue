<template>
  <div>
    ...
    <p>
      <translate
        >You can add a new wallet by selecting a provider on the list
        below.</translate
      >
      <translate
        >A wallet is a set of balances which will be displayed on the homepage
        of AMW.</translate
      >
    </p>
    <article v-if="currentProvider === null">
      ...
      <ul>
        <li
          ...
          :key="provider.network + '.' + provider.provider"
        >
          <a href="#" @click.prevent="loadProvider(provider)">
            <span class="network">{{ provider.network | capitalize }}</span
            ><br />
    ...
    <article v-else class="add-wallet">
      <h3>
        <translate :translate-params="{ provider: currentProvider.name }"
          >Add a %{provider} wallet</translate
        >
      </h3>
      <p>
        ...
        <a
          ...
          @click.prevent="$refs.modalProvider.open()"
        >
      ...
      <template v-if="currentProvider.warnings">
        <p
          v-if="currentProvider.warnings.cors"
          class="alert alert-warning"
          v-translate
        >
          This provider is missing some requirements to work in a decentralized
          way (CORS support is missing). API requests will use an intermediate
          server owned by the AllMyWallets team.
        </p>
        <p
          v-if="currentProvider.warnings.apiKey"
          class="alert alert-warning"
          v-translate
        >
          AllMyWallets is not able to check if your API key is read only. Be
          sure that the provided key does not allow any trading, withdraw or
    ...
    providers() {
      ...
      return Proxy.getProvidersList().filter(value => {
        for (const filter of this.filters) {
          if (
            filter.type === "network" &&
            value.network !== filter.name.toLowerCase()
          ) {
            return false
          }
      ...
      return this.providers
        .map(provider => {
          return {
            type: "network",
            name:
              provider.network.charAt(0).toUpperCase() +
              provider.network.slice(1)
          }
        })
        .filter(
          (value, index, array) =>
            array.map(mapObj => mapObj["name"]).indexOf(value["name"]) === index
        )
    }
  },
  methods: {
    ...
    async loadProvider(provider) {
      this.currentSchema = {
        fields: await Proxy.getProviderParameters(
          provider.network + "." + provider.provider
        )
      }
      this.currentParameters = VueFormGenerator.schema.createDefaultObject(
        this.currentSchema
      )
