<template>
  <collapsible-section icon="cogs" :class="{ warning: missing }">
    ...
    <template slot="intro">
      <p v-if="!missing" v-translate>
        Your browser is compatible with AllMyWallets!
      </p>
      <p v-else>
        <span class="text-danger" v-translate
          >Some critical requirements are missing to get all the features of
          AllMyWallets.</span
        ><br />
        <translate
          >Please consider upgrading to the last version of one of the following
          browsers:</translate
        >
