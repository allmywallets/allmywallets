<template>
  <div :class="{ balance: true, 'balance-highlighted': highlighted }">
    <holdings-chart
      :class="{ 'balance-background': true, showed: showCharts }"
    ...
    <div class="balance-content">
      <header class="balance-header">
        <span class="balance-logo" v-if="logo">
          <img
            height="40"
            :src="`/public/icons/${balance.ticker.toLowerCase()}%402x.png`"
            @error="removeLogo"
            :alt="balance.ticker"
          />
        </span>
      ...
      <div class="balance-amount">
        <small>{{ balance.ticker }}</small
        ><span :title="balance.amount" v-tippy>{{
          balance.amount | toPrecision(4)
        }}</span
        ><br />
        <balance-item-prices
...
export default {
  ...
  computed: {
    ...mapGetters(["balances", "pricesHistories", "display"]),
    balance() {
    ...
    provider() {
      const provider = Proxy.getProvidersList().find(
        provider => provider.provider === this.balance.wallet.provider
      )
    ...
    chartData() {
      const history = this.pricesHistories.find(
        history => history.ticker === this.balance.ticker
      )
