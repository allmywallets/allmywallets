<template>
  <div>
    <router-link
      v-if="missingCapabilities"
      :to="{ name: 'settings' }"
      title="Missing capabilities"
      v-tippy="{ showOnLoad: missingCapabilities }"
    >
    ...
    <a
      ...
      v-tippy="{ showOnLoad: needsRefresh }"
    >
    ...
    <a
      ...
      v-tippy="{ showOnLoad: needsUpgrade }"
    >
...
export default {
  ...
  data() {
    return {
      needsRefresh:
        this.$serviceWorker && this.$serviceWorker.controller === null
    }
  },
  computed: {
  ...
  methods: {
    ...
    upgrade() {
      return this.$store.dispatch("appUpgrade", {
